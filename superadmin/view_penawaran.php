					...
					<div class="form-row">
						<div class="col-1">
							...
							<select class="form-control" id="from" name="from">
								...
								<?php
								$resTahun = mysqli_query($conn, "SELECT tahun_penawaran FROM sales_penawaran GROUP BY tahun_penawaran");
								?>
		...
		<script>
			$(document).ready(function() {
				$('#range').click(function() {
					...
					if (from != '') {
						$.ajax({
							url: "prosescariviewpenawaran.php",
							method: "POST",
							data: {
								from: from,
								username: username
